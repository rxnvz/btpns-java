...
@RestController
@RequestMapping("/api") //buat ngeset url yang akan digunakan
public class RestAPIController {

    public static final Logger logger = LoggerFactory.getLogger(RestAPIController.class);

    @Autowired
    StaffService staffService; //deklarasi pemanggil(?) method di class StaffServiceImpl
    ...
    public ResponseEntity<?> listAllStaff() { // Represents the entire HTTP response.
        ...
        if (staffs.isEmpty()) { //kalo, arraylist staff kosong
            return new ResponseEntity<>(staffs, HttpStatus.NOT_FOUND); //nampilin(?) isi arraylist-nya dan ngasih kode not found
        }
    ...
    public ResponseEntity<?> updateStaff(@PathVariable("IDKaryawan") long IDKaryawan, @RequestBody Staff staffs) {
        ...
        if (thisStaff == null) { //kalo, staffnya ngga ada
